#!/usr/bin/env bash
# Single-expert RegNetY-1280 SWAG LoRA probe (all 1x1 convs), 518x518.
#
...
#   $RUNS/ensemble_regnety1280_518_lora_10e/
#
# Run inside tmux:
#   tmux new-session -s regnety "bash run_regnety_lora.sh"
#   # detach: Ctrl-b d     re-attach: tmux attach -t regnety
set -euo pipefail
cd "$(dirname "$0")"
...
echo
echo ">>> RegNetY-1280 SWAG LoRA (1x1 convs) run: 1 expert x 10 epochs @518px"
echo ">>> log: $OUT/train.log  (tail -f to watch)"
"$TORCHRUN" --nproc_per_node=2 --standalone \
    train_dinov3_ensemble.py \
    --data-root "$DATA" \
    --output-dir "$OUT" \
    --backbone "${BACKBONE:-regnety_1280.swag_ft_in1k}" \
