...
#   label smoothing 0.05 + mixup 0.2, early stop on val NLL (patience 3).
#
...
#
# Output: $RUNS/ensemble_dinov3_raw_native_l1_20e/  (experts/, metrics_val.json, submission.csv)
set -euo pipefail
cd "$(dirname "$0")"
...
OUT="${OUT:-$RUNS/ensemble_dinov3_raw_native_l1_20e}"
...
echo ">>> LoRA last 1 block, native res + per-batch padding, no gate"
